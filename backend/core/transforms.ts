...
export const convertToDomainStatus = (
  dbStatus: DbChoreStatus,
  updatedAt: IChore["createdAt"]
): ChoreStatus => {
  switch (dbStatus) {
...
export const convertToDomainChore = (dbChore: IChoreSchema): IChore => ({
  choreId: dbChore.id.toString(),
  createdAt: new Date(dbChore.createdAt as string),
  status: convertToDomainStatus(
    dbChore.status,
    new Date(dbChore.updatedAt as string)
  ),
  title: dbChore.title,
